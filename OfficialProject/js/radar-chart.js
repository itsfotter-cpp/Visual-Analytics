var RadarChart = {
  defaultConfig: {
    ...
    axisJoin: function(d, i) {
      return d.className != "BestPosition" || i;
    },
  ...
  chart: function() {
    ...
    function setTooltip(tooltip, msg){
      if(msg === false || msg == undefined){
        ...
        var coords = d3v3.mouse(container);
    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        var maxValue = Math.max(cfg.maxValue, d3v3.max(data, function(d) {
        ...
        // levels && axises
        var levelFactors = d3v3.range(0, cfg.levels).map(function(level) {
        ...
        var levelLine = levelGroups.selectAll('.level').data(function(levelFactor) {
          return d3v3.range(0, total).map(function() { return levelFactor; });
        });
        ...
        polygon.exit()
        .classed('d3v3-exit', 1) // trigger css transition
        .transition().duration(cfg.transitionDuration)
        .remove();

        polygon
        .each(function(d, i) {
          var classed = {'d3v3-exit': 0}; // if exiting element is being reused
          classed['radar-chart-serie' + i] = 1;
          if(d.className) {
            classed[d.className] = 1;
          }
          d3v3.select(this).classed(classed);
        })
        ...
        .each('start', function() {
          d3v3.select(this).classed('d3v3-enter', 0); // trigger css transition
        });
        ...
        if(cfg.circles && cfg.radius) {
          ...
          circleGroups.enter().append('g').classed({'circle-group': 1, 'd3v3-enter': 1});
          circleGroups.exit()
          .classed('d3v3-exit', 1) // trigger css transition
          ...
          circle
          .each(function(d) {
            var classed = {'d3v3-exit': 0}; // if exit element reused
    ...
    radar.config = function(value) {
      ...
      if(arguments.length > 1) {
        ...
        d3v3.entries(value || {}).forEach(function(option) {
